...
- APK version is `versionCode=17` and `versionName=0.5.8`.
...
- Version 0.5.8 keeps road/path reveal from being generated just because the map opens.
...
- Version 0.5.8 adds local progress-aware foreground notification text.
- The notification can rotate between fog-clearing lines, an evening summary, and a gentle return line after multiple inactive days.
- Today's cleared area is estimated from local reveal cells. Today's distance is accumulated from accepted GPS movement.
- Version 0.5.8 adds manual encrypted export/import in the info sheet.
...
Version 0.5.8 includes manual encrypted export/import. It is local and user-started,
...
access. This app does not declare that permission in version 0.5.8. See Android
