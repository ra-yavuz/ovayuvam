...
The title is under Google Play's 30-character app-title limit.
...
- No account.
- No ads.
- No ovayuva backend for your revealed world.
- No public profile.
...
Recommended: `Maps & Navigation` or `Lifestyle`.

`Maps & Navigation` is more literal because the app is a map, but the app is not a navigation tool. `Lifestyle` may be safer if the store positioning emphasizes personal exploration rather than directions.
