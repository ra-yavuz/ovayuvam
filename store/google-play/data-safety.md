...
Version 0.5.9 also stores an optional local goal pin if the user long-presses the
map. That pin is local app data and is not sent to an ovayuva backend.

Version 0.5.9 stores today's accepted walking distance and latest progress time
locally so the foreground notification can show gentle progress text.

Version 0.5.9 lets the user manually export an encrypted backup file through the
